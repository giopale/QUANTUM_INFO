...
program MatTestFixedSize
    ...
    double precision, dimension(4) :: result
    character(len=30), dimension(:), allocatable :: args
    character(len=50) :: filename, filename_alpha, filename_bravo, filename_charlie, message
    write(oo,*)
    write(oo,*) "	*** Matrix multiplication test program ***	"
    ...
    filename_alpha= trim(filename) //"_ByRows"// ".txt"
    filename_bravo = trim(filename) //"_ByCols" // ".txt"
    filename_charlie = trim(filename) // "_Intrinsic" // ".txt"

    ! Testing section 
    open(unit=10,file=filename_alpha,action='write',status='old',iostat=status)
    open(unit=20,file=filename_bravo,action='write',status='old',iostat=status)
    open(unit=30,file=filename_charlie,action='write',status='old',iostat=status)
    ! write(*,'("File opening status =" i3)') status ! Checking correct file opening
    ! write(oo, '("Filename: ",a20," Testing up to size ", i6 )') filename, nn*100
    ! write(oo,*)
    write(oo,*) "Running test..." ! courtesy message
    ...
    do ii=1,nn,1
        ...
        write(10,'(i10, G15.5)') size, time(1,ii)
        write(20,'(i10, G15.5)') size, time(2,ii)
        write(30,'(i10, G15.5)') size, time(3,ii)
    end do
        write(oo,*) "Done"
    close(10,iostat=status)
    close(20,iostat=status)
    ...
    call MatTest("suca",500,"n",result)
